document.addEventListener('DOMContentLoaded', () => {
    ...
    const galleryImages = [
        { src: "photo1.png", caption: "Every moment with you is my favorite 💕" },
        { src: "photo2.png", caption: "The perfect partner in crime 😊" },
        { src: "photo3.png", caption: "You make my world so colorful ✨" }
    ];
    const notesContent = [
    ...
    const playlistSongs = [
        { title: "Kesariya", artist: "Arijit Singh", src: "https://files.catbox.moe/r9pltd.mp3", img: "photo1.png" },
        { title: "Maan Meri Jaan", artist: "King", src: "https://files.catbox.moe/quqfx4.mp3", img: "photo2.png" },
        { title: "Apna Bana Le", artist: "Arijit Singh", src: "https://files.catbox.moe/m5qdp0.mp3", img: "photo3.png" }
    ];
    ...
    playlistSongs.forEach(song => {
        playlistWrapper.innerHTML += `
            <div class="swiper-slide playlist-slide">
                <img src="${song.img}" alt="Album Art"><h4>${song.title}</h4><p>${song.artist}</p>
    ...
    closeGameBtns.forEach(btn => btn.addEventListener('click', () => {
        gameModal.classList.add('hidden'); clearInterval(gameInterval); gameIntro.classList.remove('hidden');
        gameAreaWrapper.classList.add('hidden'); gameComplete.classList.add('hidden');
    }));
    startGameBtn.addEventListener('click', () => {
        gameIntro.classList.add('hidden'); gameAreaWrapper.classList.remove('hidden'); startGame();
    });
    function startGame() {
        score = 0; timer = 30; scoreEl.textContent = score; timeEl.textContent = timer; gameArea.innerHTML = '';
        gameInterval = setInterval(() => {
            timer--; timeEl.textContent = timer; if (timer <= 0) endGame();
            if (Math.random() > 0.1) { createGameItem('heart'); } else { createGameItem('kitty'); }
        }, 800);
    }
    function createGameItem(type) {
        const item = document.createElement('div');
        item.classList.add('game-item');
        item.innerHTML = `<img src="${type === 'heart' ? 'heart-icon.png' : 'hello-kitty.png'}">`;
        item.style.left = `${Math.random() * 85}%`; item.style.animationDuration = `${(Math.random() * 2) + 3}s`;
        item.addEventListener('click', () => {
            item.remove(); if (type === 'heart') { score++; } else { score += 3; timer += 5; }
            scoreEl.textContent = score; if (score >= 15) endGame();
        });
        gameArea.appendChild(item); setTimeout(() => { if(item) item.remove() }, 5000);
    }
    function endGame() {
        clearInterval(gameInterval); gameAreaWrapper.classList.add('hidden'); gameComplete.classList.remove('hidden');
